...
<div class="container">
    ...
    <div class="row" style="margin-top: 50px">
        <div class="col-12" style="text-align: center">
            <#if course.code = 0>
                <#if user.vip>
                    <a href="/video/learn?courseName=${course.courseName!}&courseCatalog=${course.courseCatalog!}&courseVideo=${course.courseVideo!}&code=${course.code}" class="btn btn-success">您已是会员，现在可以开始学习视频课程</a>
                <#else >
                    <a href="/recharge" class="btn btn-danger">收费课程需要开通会员，点击前往充值页面</a>
                </#if>
            <#elseif course.code = 1>
                <a href="/video/learn?courseName=${course.courseName!}&courseCatalog=${course.courseCatalog!}&courseVideo=${course.courseVideo!}&code=${course.code}"
                   class="btn btn-success">开始学习视频课程</a>
            <#elseif course.code=2>
                <a href="/video/learn?courseName=${course.courseName!}&courseCatalog=${course.courseCatalog!}&courseVideo=${course.courseVideo!}&code=${course.code}" class="btn btn-success">限时免费课程，快去学习吧</a>
            </#if>
        </div>
    </div>
</div>
